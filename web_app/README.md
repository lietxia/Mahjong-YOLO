...
这个目录是 Mahjong YOLO 的浏览器端推理应用。当前对外可见能力主要包括：

- 上传图片推理
- Python 基线比对
- 手牌排序与麻将计分
- Web Worker 后台推理
- 多模型切换（当前默认模型 + Large 模型）
- 画布展示全部后处理检测框
...
- **更完整的状态反馈**：页面会解释空状态、初始化中、回退中、失败场景与模型兼容性。
- **模型切换**：支持在多个 ONNX 模型之间切换，并在推理前按需初始化当前所选模型。
...
2. 为生产环境启用 **HTTPS**。
3. 不要让静态服务器拦截或重写 `model/*.json` 和 `model/*.onnx` 请求。
4. 如果未来重新启用摄像头能力，再额外保证 HTTPS 或 localhost。
...
- 页面会进一步区分：
  - 浏览器未提供 WebGPU API
  - 浏览器已提供 WebGPU API，但当前拿不到 GPU adapter

### 当前产品路径说明

当前主产品路径已经收敛为“上传图片推理”。仓库里仍保留部分历史实现或辅助代码，但前端主 UI 不再暴露摄像头识别入口。
...
4. 构建后 `dist/model/` 下的所有启用模型（包括 `mahjong-yolol-best.onnx`）是否被正确复制。
